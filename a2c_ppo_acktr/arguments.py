...
def get_args():
    ...
    parser.add_argument("--kl-beta", type=float, default=0.04, help="kl beta (default: 0.04)")
    ...
    parser.add_argument("--stop_grad", type=str, default="yes", help="propagate gradient from value head to model")
    parser.add_argument(
        "--freeze-policy-model-on-first-update",
        action="store_true",
        default=False,
        help="freeze policy model on first update",
    )
    parser.add_argument(
        "--value-warmup",
        type=str,
        default="yes",
        help="value warmup (default: no)",
    )
    parser.add_argument(
        "--use-kl",
        type=str,
        default="yes",
        help="use kl (default: no)",
    )
    parser.add_argument(
        "--max-episode-steps",
        type=int,
        default=128,
        help="maximum number of environment steps (default: 128)",
    )
    parser.add_argument(
        "--max-image-obs-len",
        type=int,
        default=4,
        help="maximum length of image observation (default: 10)",
    )
    parser.add_argument(
        "--init-lr",
        type=float,
    ...
    parser.add_argument(
        "--end-lr", type=float, default=1e-8, help="final learning rate (default: 1e-8)"
    )
    parser.add_argument(
        "--weight-decay", type=float, default=0, help="weight decay (default: 1e-3)"
    )
    parser.add_argument(
        "--explore_portion",
        type=float,
    ...
    parser.add_argument(
        "--prompt",
        type=str,
        default="What is the next action? Please format your response as 'The next action is \{response\}.'",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="any number between 0-1, multiplier for the log thought probability",
    )
